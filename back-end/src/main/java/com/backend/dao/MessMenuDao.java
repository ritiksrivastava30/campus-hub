...
public class MessMenuDao extends StarterDao{
	...
	public Day updateMenu(String hostelName,String day,String time,String updatedItem) {
...
//			return fetchMessMenu(hostelName);
			query="SELECT `day`,`breakfast`,`lunch`,`dinner` from `messmenu` WHERE (`hostel_id` = ? AND `day` = ? AND `time` = ?);";
			return jdbcTemplate.queryForObject(query, new MessMenuRowMapper(), hostel_id, day,time);;
		}catch(Exception e) {
